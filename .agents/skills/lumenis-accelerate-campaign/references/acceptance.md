...
- Workflow: intended forms/segments enroll, correct division branches and Salesforce campaign/status, Latest Lead Info set as the first action (before any delay or Salesforce action), the form added to the corporate recent-conversion workflow's exclusion list, and the value confirmed in the Salesforce new-lead email, delay/Capture Date reviewed, correct email selected, unknown routing handled, existing matches and re-enrollment explicitly decided. Saved OFF/ON state read back.
